...
resource "oci_database_autonomous_database" "ATPdatabase" {
  ...
  nsg_ids                  = [oci_core_network_security_group.ATP_nsg.id]   
  private_endpoint_label   = var.ATP_database_atp_private_endpoint_label
  subnet_id                = oci_core_subnet.ATPsubnet.id      
}
